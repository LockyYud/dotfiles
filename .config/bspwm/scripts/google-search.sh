...
# Theme settings
THEME="-theme ~/.config/rofi/themes/google-search.rasi"
...
main() {
  ...
  platform=$( (gen_list) | rofi $THEME -dmenu -matching fuzzy -no-custom -location 0 -p "Search > " )

  if [[ -n "$platform" ]]; then
    query=$( (echo ) | rofi $THEME -dmenu -matching fuzzy -location 0 -p "Query > " )
