...
parse_keys() {
  ...
  # shorten some key names
    ...
    bracketright) key="]" ;;
        ...
        case "$cmd" in
          /exit)
            (eww -c "$basedir/bubbles" close bubbly; pkill -f "eww/bubbly/bubbles/scripts/getkeys.sh") &
            exit 0
            ;;
          /clear)
            ~/.config/eww/AI/assistant.py clear_history
            ;;
          /help)
            ~/.config/eww/AI/assistant.py inject_help
            ;;
          *)
            if [ -n "$msg" ]; then
              ~/.config/eww/AI/assistant.py chat "$msg" >>"$HOME/.config/eww/AI/assistant.log" 2>&1 &
            fi
            ;;
        esac
